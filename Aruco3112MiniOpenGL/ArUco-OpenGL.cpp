...
// Drawing function
void ArUco::drawScene(Objet zeldo, Objet meteor) {
   ...
   for (unsigned int m=0;m<m_Markers.size();m++)
   {
	  ...
	  if (m_Markers[m].id==playerMarker) {
		  float div = 50.0f;
		  
		  glEnable(GL_TEXTURE_2D);
		  glBindTexture(GL_TEXTURE_2D, zeldo.getTextureID());
		  zeldo.drawObjet();
		 
	  }
	  
	  if (m_Markers[m].id == meteorMarker) {

		  vector<Vector3> met = meteorManager.GetMeteorCoordonateManager();
		  
		  glEnable(GL_TEXTURE_2D);
		  glBindTexture(GL_TEXTURE_2D, meteor.getTextureID());
		  for (int i = 0; i < met.size(); i++) {
			  glPushMatrix();
			  
			  glTranslatef(met[i].getX(), met[i].getY(), met[i].getZ());
			  meteor.drawObjet();
			  glPopMatrix();
		  }
		  meteor.drawObjet();
...
void ArUco::idle(Mat newImage) {
   ...
   if (meteorMarker == 0 && m_Markers.size()>=1) {
	   meteorMarker = m_Markers[0].id;
	   
   }
   if (playerMarker == 0 && m_Markers.size() >= 2) {
	   if (m_Markers[0].id != meteorMarker) {
		   playerMarker = m_Markers[0].id;
	   }
	   else {
		   playerMarker = m_Markers[1].id;
	   } 
   }


   cout << meteorManager.GetMeteorCoordonateManager().size() << endl;
   if (timeManager.getTime() % 100 == 0) {
	   ...
	   cout << "adding new meteor" << endl << endl << endl << endl << endl << endl << endl << endl << endl << endl << endl << endl << endl << endl << endl << endl << endl << endl << endl << endl << endl << endl << endl << endl << endl << endl << endl << endl << endl;
	   meteorManager.addMeteor(randomX, randomY, 0.3f);
   }
